...
/*
		...
		gl_FragColor = vec4(normalize(eyespacenormal),1.0);
	#else
		gl_FragColor = vec4(normalize(normal),1.0);
	#endif
*/
	#ifdef COLOR_MAP
		gl_FragColor = texture2D(colormap, gl_TexCoord[0].st) * gl_FrontMaterial.diffuse;
	#else
		#ifdef SPLATTING
			vec4 color;
			...
			color.a = 1.0;
			gl_FragColor = color * gl_FrontMaterial.diffuse;
		#else
