...
void main()
{
	vec4 shadow = texture2D(shadow_map, gl_FragCoord.st / screen_size);
	...
	gl_FragData[0] = color*ambient + shadow.r * (color*diffuse + specular*diffuse_factor);
	gl_FragData[1] = (color*ambient + shadow.r * (color*diffuse + specular*diffuse_factor)) * shadow.r * specular;
}
