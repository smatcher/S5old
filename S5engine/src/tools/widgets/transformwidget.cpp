...
TransformWidget::TransformWidget(Transformf &transform) : QGroupBox(tr("Transform")), m_transform(transform)
{
	...
	m_x = new QLineEdit(QString().setNum(pos.x,'f',3));
	m_x->setDisabled(true);
	m_y = new QLineEdit(QString().setNum(pos.y,'f',3));
	m_y->setDisabled(true);
	m_z = new QLineEdit(QString().setNum(pos.z,'f',3));
	m_z->setDisabled(true);


	Vector3f rot = m_transform.toEuler();
	m_rx = new QLineEdit(QString().setNum(rot.x,'f',3));
	m_rx->setDisabled(true);
	m_ry = new QLineEdit(QString().setNum(rot.y,'f',3));
	m_ry->setDisabled(true);
	m_rz = new QLineEdit(QString().setNum(rot.z,'f',3));
	m_rz->setDisabled(true);

	Vector3f scale = m_transform.getScale();
	m_sx = new QLineEdit(QString().setNum(scale.x,'f',3));
	m_sx->setDisabled(true);
	m_sy = new QLineEdit(QString().setNum(scale.y,'f',3));
	m_sy->setDisabled(true);
	m_sz = new QLineEdit(QString().setNum(scale.z,'f',3));
...
{
	Vector3f pos = m_transform.getPosition();
	m_x->setText(QString().setNum(pos.x,'f',3));
	m_y->setText(QString().setNum(pos.y,'f',3));
	m_z->setText(QString().setNum(pos.z,'f',3));

	Vector3f rot = m_transform.toEuler();
	m_rx->setText(QString().setNum(rot.x,'f',3));
	m_ry->setText(QString().setNum(rot.y,'f',3));
	m_rz->setText(QString().setNum(rot.z,'f',3));

	Vector3f sca = m_transform.getScale();
	m_sx->setText(QString().setNum(sca.x,'f',3));
	m_sy->setText(QString().setNum(sca.y,'f',3));
	m_sz->setText(QString().setNum(sca.z,'f',3));
}
