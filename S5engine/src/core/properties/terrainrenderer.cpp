...
void TerrainPatch::drawAdaptedEdge(int nx, int ny, int edge) {
	...
	if(lod)
		glDrawElements(GL_TRIANGLES, PATCH_LORES_EDGE_VBO_SIZE, GL_UNSIGNED_INT, 0);
	else
		glDrawElements(GL_TRIANGLES, PATCH_HIRES_EDGE_VBO_SIZE, GL_UNSIGNED_INT, 0);
...
TerrainRenderer::TerrainRenderer(Texture& hm, Material& mat, float yscale, float scale, float tscale): IRenderable("TerrainRenderer"),
	...
	/* Pas terrible, vue que la Texture n'est pas FORCEMENT une StbImage,   am̩liorer donc... */
